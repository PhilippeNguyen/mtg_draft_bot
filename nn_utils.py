...
def build_model(input_size, num_dense=3):
    ...
    cur_pick = KL.Lambda(lambda x: x[:, set_size:], output_shape=(set_size,))(
        input_layer
    )
...
class NNBot(object):
    ...
    def pick_and_add(self, pack):
        ...
        if card_idx not in out_pack:
            warnings.warn(
                f" Bot picked none of the cards \
             in pack. This may happen if the pack contains \
             only cards the bot hasnt seen during training \
             (e.g. basic lands used during simulation, but not in training). \
             Will pick the first remaining card in pack instead :  {out_pack}"
            )
            card_idx = out_pack[0]
        self.add(card_idx)
