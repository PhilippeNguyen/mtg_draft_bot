...
class ScryfallDraftCreator(BaseDraftCreator):
    def __init__(self, set_df, set_code):
        # TODO: need to handle set_df
        self.set_df = set_df
        self.set_code = set_code
        self.mythic_prob = 1 / 7.4
        self.num_common = 10
        self.num_uncommon = 3
        self.num_rare = 1
        self.num_land = 1
    ...
    def build_set(self):
        # TODO: build set from set_df, and implement read_pool_picks and create_pack
        self.mythics = []
        self.rares = []
        self.uncommons = []
        self.commons = []
        ...
        for row_idx, row in self.set_df.iterrows():
            if (
                row["Type"].lower().startswith("basic land")
                or row["Type"].lower() == "land"
            ):
                self.lands.append(row_idx)
            elif row["Rarity"].lower() == "mythic":
                self.mythics.append(row_idx)
            elif row["Rarity"].lower() == "rare":
                self.rares.append(row_idx)
            elif row["Rarity"].lower() == "uncommon":
                self.uncommons.append(row_idx)
            elif row["Rarity"].lower() == "common":
                self.commons.append(row_idx)
            else:
                raise Exception("Unknown rarity: {}".format(row["Rarity"]))
    ...
    def create_pack(self):
        pack = []
        # add commons
        pack.extend(np.random.choice(self.commons, size=self.num_common, replace=False))
        # add uncommons
        pack.extend(
            np.random.choice(self.uncommons, size=self.num_uncommon, replace=False)
        )
        # add rare or mythic
        if np.random.uniform() < self.mythic_prob:
            pack.extend(
                np.random.choice(self.mythics, size=self.num_rare, replace=False)
            )
        else:
            pack.extend(np.random.choice(self.rares, size=self.num_rare, replace=False))

        # add lands
        pack.extend(np.random.choice(self.lands, size=self.num_land, replace=False))
...
class STXDraftCreator(ScryfallDraftCreator):
    def __init__(self, set_df=None, set_code=None):
        self.set_code = "stx"
        self.pack_size = 15
    ...
    def create_pack(self):
        ...
        pack.extend(
            np.random.choice(
                self.idx_dict["stx_c"], size=self.num_common_slot, replace=False
            )
        )

        # add the lesson slot
        lesson_draw = np.random.multinomial(
            1, [self.common_lesson_rate, self.rare_lesson_rate, self.mythic_lesson_rate]
        )
        if np.argmax(lesson_draw) == 0:
            pack.extend(
                np.random.choice(self.idx_dict["lesson_c"], size=1, replace=False)
            )
        elif np.argmax(lesson_draw) == 1:
            pack.extend(
                np.random.choice(self.idx_dict["lesson_r"], size=1, replace=False)
            )
        elif np.argmax(lesson_draw) == 2:
            pack.extend(
                np.random.choice(self.idx_dict["lesson_m"], size=1, replace=False)
            )

        # add 2 uncommons
        pack.extend(
            np.random.choice(
                self.idx_dict["stx_u"], size=self.num_uncommon_slot - 1, replace=False
            )
        )
        ...
        if np.random.uniform() < self.uncommon_lesson_swap_rate:
            pack.extend(
                np.random.choice(self.idx_dict["lesson_u"], size=1, replace=False)
            )
        else:
        ...
        # add sta slot
        sta_draw = np.random.multinomial(
            1, [self.uncommon_sta_rate, self.rare_sta_rate, self.mythic_sta_rate]
        )
...
class M19DraftCreator(BaseDraftCreator):
    ...
    def _get_default_set_df(self):
        return pd.read_csv(
            pjoin(metadata_path, "M19_standardized_rating.tsv"), delimiter="\t"
        )
    ...
    def create_pack(self):
        ...
        for _ in range(self.num_rare):
            if np.random.uniform() < self.mythic_prob:
                pack.extend(
                    np.random.choice(self.rarities_idx["M"], size=1, replace=False)
                )
            else:
                pack.extend(
                    np.random.choice(self.rarities_idx["R"], size=1, replace=False)
                )
        pack.extend(
            np.random.choice(
                self.rarities_idx["U"], size=self.num_uncommon, replace=False
            )
        )
        pack.extend(
            np.random.choice(
                self.rarities_idx["C"], size=self.num_common, replace=False
            )
        )
...
class ScryfallMetaData(BaseSetMetaData):
    def __init__(self, set_code=None):
        self.draft_creator = ScryfallDraftCreator
        self.set_code = set_code

    @property
    def set_df_path(cls):
        return pjoin(metadata_path, f"{cls.set_code.upper()}_card_names.csv")

    @property
    def image_folder_path(cls):
        return pjoin(images_path, cls.set_code.lower())

    def load_draft_creator(self):
        set_df = pd.read_csv(self.set_df_path)
        return self.draft_creator(set_df=set_df, set_code=self.set_code)


class STXMetaData(ScryfallMetaData):
...
available_sets = ["m19", "stx", "afr", "mid", "vow"]
set_metadata_map = {
    "m19": M19MetaData,
    "stx": STXMetaData,
    "afr": ScryfallMetaData("afr"),
    "mid": ScryfallMetaData("mid"),
    "vow": ScryfallMetaData("vow"),
}
