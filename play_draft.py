...
class MainApplication(tk.Frame):
    def __init__(self, root, flag_set=None, flag_model=None):
        ...
        filemenu.add_command(label="New Draft", command=self.popNewDraftWindow)
        filemenu.add_command(label="Exit", command=self.popQuitWindow)
        ...
        view_menu.add_command(label="View Bot/Player", command=self.popChangeViewMenu)
    ...
    def apply_options(self):
        ...
        blank_image = resize_image(
            blank_image, (self.card_image_width.get(), self.card_image_height.get())
        )
        self.blank_image = ImageTk.PhotoImage(blank_image)
        black_image = Image.open(self.black_image_path)
        black_image = resize_image(
            black_image, (self.card_image_width.get(), self.card_image_height.get())
        )
    ...
    def init_set(self):
        ...
        self.deck_frame.grid(
            rowspan=self.num_card_rows.get(), row=0, column=self.deck_col
        )
        ...
        self.canvas_scrollbar.grid(
            row=self.canvas_row, column=self.scrollbar_col, sticky="nse"
        )
        ...
        self.canvas.grid(row=self.canvas_row, column=self.canvas_col)
    ...
    def start_draft(self):
        ...
        for _ in range(self.num_bots):
            nnbot = NNBot(
                keras_model=self.model, set_size=self.set_size, pick_mode="max"
            )
    ...
    def pick_rotation(self):
        ...
        for card_idx in self.player_pack:
            if card_idx == -1:
                continue
            card_infos.append(
                (card_idx, self.draft_creator.get_info_from_idx(card_idx))
            )

        self.player_card_tuples = sorted(
            card_infos,
            key=lambda x: -self.draft_creator.rarity_ordering[x[1]["Rarity"]],
        )
    ...
    def display_cards(self):
        ...
        for button_idx, _ in enumerate(card_tuples):
            if button_idx < len(card_tuples):
                ...
                image_path = pjoin(
                    self.set_images_folder, "{:04d}.jpg".format(card_idx)
                )
                card_image = Image.open(image_path)
                card_image = resize_image(
                    card_image,
                    (self.card_image_width.get(), self.card_image_height.get()),
                )

                if card_pick is not None and card_pick == card_idx:
                    card_image = ImageOps.expand(
                        card_image, border=10, fill=(255, 0, 255)
                    )
        ...
        min_height = (
            self.root.winfo_screenheight() - self.display_label.winfo_height() - 80
        )
        self.canvas.configure(
            ...
            height=min(
                min_height, self.num_card_rows.get() * self.card_image_height.get()
            ),
        )
...
class ChangeSettings(tk.Frame):
    def __init__(self, parent, main, *args, **kwargs):
        ...
        self.scale_slider = tk.Scale(
            self, orient="horizontal", from_=0.25, to=2.0, resolution=0.05
        )
    ...
    def return_to(self):
        ...
        self.mainApp.card_image_height.set(
            self.mainApp.scale.get() * self.mainApp.init_height
        )
        self.mainApp.card_image_width.set(
            self.mainApp.scale.get() * self.mainApp.init_width
        )
...
class ChangeView(tk.Frame):
    ...
    def return_to(self):
        self.mainApp.cur_view_idx.set(
            self.mainApp.view_map[self.mainApp.cur_view_name.get()]
        )
        self.mainApp.display_cards()
        self.mainApp.update_display()
        self.parent.destroy()
...
class QuitWindow(tk.Frame):
    def __init__(self, parent, main, *args, **kwargs):
        ...
        self.label.pack()
        self.quitButton = tk.Button(self, text="Yes", width=25, command=self.quit)
        self.quitButton.pack(side="left")
        self.returnButton = tk.Button(self, text="No", width=25, command=self.return_to)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="set_code",
        default=None,
        help=("3-symbol code for the set "),
    )
    args = parser.parse_args()
    root = tk.Tk()
    MainApplication(root, flag_set=args.set_code, flag_model=args.model_hdf5).pack(
